...
fn score_with_double(double_score: f64) -> OneOfScore {
    OneOfScore { score : Some(one_of_score::Score::DoubleScore(double_score)) }
}

fn boolean_testcases(testcases: &Vec<TestcaseResult>) -> bool {
    testcases.iter().all(|testcase_result| {
        match testcase_result.score.score {
            Some(one_of_score::Score::BoolScore(_)) => true, 
            _ => false
        }
    })
}

fn double_testcases(testcases: &Vec<TestcaseResult>) -> bool {
    testcases.iter().all(|testcase_result| {
        match testcase_result.score.score {
            Some(one_of_score::Score::DoubleScore(_)) => true, 
            _ => false
        }
    })
}
...
pub fn evaluate_submission_score(
